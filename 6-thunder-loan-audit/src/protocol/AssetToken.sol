...
contract AssetToken is ERC20 {
    ...
    // e underlying == asset that is deposited ie USDC
    // e asset == token minted in exchange for underlying deposit ie vUSDC
    // e it is the rate between the underlying asset and the token
    ...
    // e only the thunderloan contract can mint, burn etc
    ...
    function transferUnderlyingTo(address to, uint256 amount) external onlyThunderLoan {
        // @followup weird erc20s?
        // qanswered what happens if usdc blacklists the thunderloan contract or asset token contract?
    ...
    function updateExchangeRate(uint256 fee) external onlyThunderLoan {
        ...
        // @follup q why? ^invariant
        // newExchangeRate = oldExchangeRate * (totalSupply + fee) / totalSupply
        // newExchangeRate = 1 (4 + 0.5) / 4
        // newExchangeRate = 1.125

        // q what if totalSupply is 0?
        // then it would break? is that an issue?
