...
contract DeployVaultGuardians is Script {
    function run() external returns (VaultGuardians, VaultGuardianGovernor, VaultGuardianToken, NetworkConfig) {
        ...
        // xq what is the point in the VaultGuardians being the owner of the token?
        // a because VaultGuardiansBase (which VaultGuardians is) mints them to vaultGuardians
        // xq when does the owner contract call mint()?
        // a the mint() is used in VaultGuardiansBase
