...
contract VaultGuardiansBase is AStaticTokenData, IVaultData {
    ...
    // e guardians address is mapped to the asset which is mapped to the vault shares contract?
    mapping(address guardianAddress => mapping(IERC20 asset => IVaultShares vaultShares)) private s_guardians;
    // xq what is the point in this mapping? where is it actually being used?
    // @written-info this mapping is not needed because it isnt being used
    ...
    // @written-info this event isnt used in this contract
    ...
    /*//////////////////////////////////////////////////////////////
                               MODIFIERS
    //////////////////////////////////////////////////////////////*/
    // e this mapping of the guardian address to the token which is mapped to the vault shares is checked here
     ...
     */
    // @written-followup, if they have to send an ETH amount equal to the fee, why is this not payable?
    function becomeGuardian(AllocationData memory wethAllocationData) external returns (address) {
        // e when someone becomes a guardian a new VaultShares contract is deployed
    ...
    // xq does someone who wants to be a token guardian have to call becomeGuardian first, depositing WETH?
    // a yes they do
    // xq what happens if a guardian passes the weth address to this function?
    // a doesnt work because it reverts because weth is not tokenOne or tokenTwo
        ...
        returns (address)
    {
        ...
        if (address(token) == address(i_tokenOne)) {
            ...
            // xq is this meant to be deploying TOKEN_ONE_VAULT_NAME/SYMBOL???? shouldnt it be TOKEN_TWO??
            // a it should be TOKEN_TWO
            // @written-low incorrect vault name/symbol
            ...
            tokenVault = new VaultShares(
                IVaultShares.ConstructorData({
                    ...
                    guardianAndDaoCut: s_guardianAndDaoCut,
                    vaultGuardians: address(this),
                    // @answered-followup what about link??
                    // a link is not passed to the uniswap adapter
                    weth: address(i_weth),
                    usdc: address(i_tokenOne)
                })
            );
     ...
     * @notice allows a guardian to quit
        // @skipped-followup test this invariant 
    ...
    function _becomeTokenGuardian(IERC20 token, VaultShares tokenVault) private returns (address) {
        // @written-info lots of storage reads s_guardianStakePrice
        s_guardians[msg.sender][token] = IVaultShares(address(tokenVault));
        emit GuardianAdded(msg.sender, token);
        // xq are we supposed to be minting the s_guardianStakePrice amount?
        // a yes
        // xq is there a reentrancy type of vulnerability here?
        // a dont think so
        i_vgToken.mint(msg.sender, s_guardianStakePrice);
        // xq why do we need to mint to the guardian rather than minting directly to address(this)?
        // a dumb question, the shares token and the asset token are different
    ...
    // @written-info this mapping isnt being used for anything!
