...
contract L1BossBridge is Ownable, Pausable, ReentrancyGuard {
    ...
    // @written-info should be constant
    uint256 public DEPOSIT_LIMIT = 100_000 ether; // e depositing tokens limit - can't do too many

    IERC20 public immutable token; // e one bridge per token
    L1Vault public immutable vault;
    mapping(address account => bool isSigner) public signers; // users who can "send" token from L2 -> L1
    ...
    // @written-high if a user approves the bridge, any other user can steal their funds
    // Alice: approve token -> bridge
    // Bob: depositTokensToL2(from: Alice, l2Recipient: Bob, amount: all her money)

    // @written-high if the vault approved the bridge, can a user steal funds from the vault?
    function depositTokensToL2(address from, address l2Recipient, uint256 amount) external whenNotPaused {
        ...
        // @written-info should follow CEI
     ...
     */
    // @written-written signature replay attack
    // protect against it by using a nonce or deadline,
    // ie some parameters that ensure when it gets hashed it can only be used once
    ...
    function sendToL1(uint8 v, bytes32 r, bytes32 s, bytes memory message) public nonReentrant whenNotPaused {
        ...
        // q slither said this is bad, @audit-followup
        // @audit-high attacker can send data causing this contract to approve the attacker in the vault
        // @written-medium/low gas bomb. attacker can send data with high gas costs, impacting signers
